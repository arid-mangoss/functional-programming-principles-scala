...
object Anagrams extends AnagramsInterface {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.toLowerCase
      ...
      .map((k, v) => (k, v.length))
      .toList
      .sortWith((c1, c2) => c1._1 < c2._1)

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = wordOccurrences(
    s.mkString
  )
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] =
    dictionary.groupBy((w: Word) => wordOccurrences(w)).withDefaultValue(Nil)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    occurrences match {
      ...
      case head :: tl => {
        (for {
          i <- (0 to head._2).toList
          comb <- combinations(tl)
        } yield (head._1, i) :: comb).map(combList =>
          combList.filter(occur => occur._2 != 0)
        )
      }
    }
  ...
  /** Returns a list of all anagram sentences of the given sentence.
    ...
    *   - any order is fine as long as all the anagrams are there. Every
    *     returned word has to exist in the dictionary.
    *
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def recur(occurences: Occurrences): List[Sentence] =
      occurences match {
        case Nil => List(List())
        case _ => {
          for {
            occurrence <- combinations(occurences)
            if (dictionaryByOccurrences.contains(occurrence))
            word <- dictionaryByOccurrences(occurrence)
            rest <- recur(subtract(occurences, occurrence))
          } yield word :: rest
        }
      }
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(sentenceOccurrences("Robert" :: "Hello" :: Nil))
    println(dictionaryByOccurrences)
    println(combinations(List(('a', 2), ('b', 2))))
    val lard = List(('a', 2), ('d', 2), ('l', 2), ('r', 2))
    val r = List(('r', 2), ('d', 2))
    val lad = List(('a', 2), ('d', 2), ('l', 2))
